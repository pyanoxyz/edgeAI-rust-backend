...
impl DBConfig {
    ...
    pub fn store_children_context(
        ...
        info!(
            "Storing {} in the context_children with user_id {} parent_path {}",
            vec_row_id,
            user_id,
            parent_path
        );
    ...
    pub fn delete_children_context_by_parent_path(
        ...
        let vec_row_ids: Vec<u64> = {
            ...
            let ids = stmt
                .query_map(params![user_id, session_id, parent_path], |row| row.get(0))?
    ...
    pub fn delete_children_context_by_file_path(
        ...
        let vec_row_ids: Vec<u64> = {
            ...
            let ids = stmt
                .query_map(params![user_id, session_id, file_path], |row| row.get(0))?
                .filter_map(Result::ok)
                .collect();
            ids
        };
    ...
    pub fn fetch_path_session(
        ...
        // Execute the query and iterate over the rows, collecting them into the vector
        let result = stmt.query_row([user_id, session_id, parent_path], |row| {
            Ok(
                json!({
                    "user_id": row.get::<_, String>(0)?,  // user_id
                    "session_id": row.get::<_, String>(1)?,  // session_id
    ...
    pub fn update_session_context_timestamp(
        &self,
        user_id: &str,
        session_id: &str,
        parent_path: &str
