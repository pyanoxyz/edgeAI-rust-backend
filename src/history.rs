...
pub async fn get_history_by_session(req: HttpRequest, path: web::Path<String>) -> Result<HttpResponse, Error> {
    let session_id = path.into_inner();
    // Check session and extract user ID from the request
    match is_request_allowed(req.clone()).await {
        Ok(Some(user)) => {
            debug!("Ok reached here");
            let results = DB_INSTANCE.fetch_chats_for_session_and_user(&session_id, &user.user_id);
            Ok(HttpResponse::Ok().json(results))
        }
        Ok(None) => {
            let results = DB_INSTANCE.fetch_chats_for_session_and_user(&session_id, "user_id");
            Ok(HttpResponse::Ok().json(results))
        }
        Err(e) => {
            // Error handling with InternalError response
