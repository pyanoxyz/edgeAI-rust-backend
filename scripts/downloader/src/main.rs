...
fn download_from_json(json_url: &str) -> BoxResult<Vec<DownloadResult>> {
    ...
    let response: JsonResponse = response.json()
    ...
    let progress_bar = Arc::new(Mutex::new(ProgressBar::with_draw_target(Some(total_size), progress_target)));
    ...
    for handle in handles {
        if let Ok(result) = handle.join().map_err(|_| DownloadError::Other("Thread panicked".to_string()))? {
            results.push(result);
        }
    }
...
fn main() -> BoxResult<()> {
    ...
    if args.len() != 2 && args.len() != 3 {
        return Err(DownloadError::Other("Usage: program <json_url> or program <url> <file_path>".to_string()));
    }
    
    if args.len() == 2 {
        // JSON URL mode
        let json_url = &args[1];
        let results = download_from_json(json_url)?;
        
        let mut has_error = false;
        for result in results {
            match result.error {
                Some(_err) => has_error = true,
                None => {}
            }
        }
        if has_error {
       ...
       .map_err(|e| DownloadError::Request(format!("Failed to fetch file info: {}", e)))?;
        ...
        let result = download_single_file(&client, url, file_path, Some(Arc::new(Mutex::new(progress_bar))))?;
        let mut has_error = false;
